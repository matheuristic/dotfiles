...
#+end_example
* Install proselint
[[https://github.com/amperser/proselint][Proselint]] is a linter for English prose. Proselint can be configured using the =~/.config/proselint/config= file.
#+begin_example
conda create -n proselint
conda activate proselint
conda install proselint
conda deactivate
cd ~/.local/bin
ln -s ~/miniconda3/envs/proselint/bin/proselint
#+end_example
* Install ShellCheck
[[https://github.com/koalaman/shellcheck][ShellCheck]] is a static analyzer for Bash and sh scripts.
#+begin_example
conda create -n shellcheck
conda activate shellcheck
conda install shellcheck
conda deactivate
cd ~/.local/bin
ln -s ~/miniconda3/envs/shellcheck/bin/shellcheck
#+end_example
* Install fonts
