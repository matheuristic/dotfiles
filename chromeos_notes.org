...
* Install uni (optional)
[[https://github.com/arp242/uni][uni]] is a command-line tool for querying the Unicode database. It can
be used to identify codepoints in a string, search for codepoints,
print codepoints by class/block/range, and find emojis. Binary
releases are available and can be installed as below (modify the
version, architecture and paths as desired).
#+begin_example
mkdir -p $HOME/packages/uni
cd $HOME/packages/uni
wget https://github.com/arp242/uni/releases/download/v2.2.1/uni-v2.2.1-linux-amd64.gz
gunzip uni-v2.2.1-linux-amd64.gz
ln -s uni-v2.2.1-linux-amd64 uni
cd $HOME/.local/bin
ln -s $HOME/packages/uni/uni
#+end_example
* Configure git
Get ~git-credential-netrc~ and put it somewhere on ~$PATH~.
#+begin_example
...
#+end_example
Create a ~$HOME/.authinfo~ file with credentials in the following format:
#+begin_example
machine some.server.com login some_login password some_password protocol https
machine another.server.com login another_login password another_password protocol https
#+end_example
Create or import a GPG key (not shown here). Create an encrypted
version of the credentials file ~$HOME/.authinfo.gpg~ and delete the
unencrypted version.
#+begin_example
gpg -e -r the_recipient $HOME/.authinfo
rm $HOME/.authinfo
#+end_example
Configure git to use the encrypted credentials locally in a
repository. Change ~--local~ to ~--global~ to use the encrypted
credentials globally.
#+begin_example
git config --local credential.helper "netrc -f ~/.authinfo.gpg -v"
#+end_example
Adapted from [[https://grahamlopez.org/git/git_credentials.html][here]] and [[https://andrearichiardi.com/blog/posts/git-credential-netrc.html][here]].
