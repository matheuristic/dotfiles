...
#+end_example
Create a ~$HOME/.netrc~ file with credentials as follows. Note that
Emacs [[https://www.gnu.org/software/emacs/manual/html_mono/auth.html][auth-source]] users may want to use ~$HOME/.authinfo~ instead, in
