...
** Emacs package archive signature check errors

When the GNU ELPA package archive signature keys are too old, there
may be errors like ~Failed to verify signature archive-contents.sig~
or similar.

In that case, the keys needs to be refreshed.
One method is to install the ~gnu-elpa-keyring-update~ package.

1. Set the variable ~package-check-signature~ to ~nil~ by doing
   ~M-: (setq package-check-signature nil) RET~.
2. Do ~M-x package-list-packages~ and the package listing should
   refresh now.
3. Install the ~gnu-elpa-keyring-update~ package.
4. Reset the ~package-check-signature~ variable to its default by
   doing ~M-: (setq package-check-signature 'allow-unsigned) RET~ or
   by restarting Emacs.
...
- ~nonelpa~: Uses a package from NonGNU ELPA.
...
* Set paths and environment variables                       :semiearly:melpa:

Set ~exec-path~ and the environment variables.
...
[[https://git.savannah.gnu.org/cgit/emacs.git/tree/admin/notes/tree-sitter/starter-guide?h=feature/tree-sitter][Tree-sitter]] is an incremental parsing system. Other packages can build
on it to provide functionality that depends on code structure.

[[https://blog.phundrak.com/emacs-29-what-can-we-expect/#tree-sitter-is-also-part-of-emacs-core][More info]].
...
*** transpose-frame for rotating frames                            :vendored:
...
** Eat (Emulate A Terminal)                                         :nongnu:

[[https://codeberg.org/akib/emacs-eat][Eat]] is a fast terminal emulator implemented purely in Emacs Lisp, with
Sixel, mouse, and many other integrations.

To add Bash shell integration, add the following to ~$HOME/.bashrc~:
...
To add ZSH shell integration, add the following to ~$HOME/.zshrc~:
...
** View and compare directory trees using Ztree                        :gnu:
...
** Display available bindings in a popup                               :gnu:
...
** Expanding selected regions by semantic units                        :gnu:
...
** Iedit mode for editing occurances of the same word simultaneously :nongnu:
...
** Structured editing using Paredit                                 :nongnu:
...
The default ~undo~ behavior of Emacs moves backwards in time and not
edits, so if you undo and want to redo, you need to undo the undo.

We can use ~undo-only~ and ~undo-redo~ to get behavior that is more
consistent with the usual undo semantics for most applications.
That is, ~undo-only~ moves backwards in edits and ~undo-redo~ moves
forwards in edits.

The following overwrites the usual ~undo~ keybinding ~C-/~ to use
~undo-only~ (note that by default, ~undo-redo~ is bound to ~C-?~
so we need not bind it).
...
** Visualize and traverse the undo tree                                 :gnu:

Undo history can be visualized as tree using [[https://github.com/casouri/vundo][vundo]] which also lets
users move in the tree to previous states. This can be useful to get
to some branch state in the past, especially when the default undo
behavior has been changed to a linear form (see above).

The following code loads the package, and rebinds ~C-x u~ to show the
undo tree using vundo.
...
[[https://editorconfig.org/][EditorConfig]] is a tool to maintain consistent style across editors, using
~.editorconfig~ files, including for [[https://github.com/editorconfig/editorconfig-emacs][Emacs]].
...
** csv-mode for comma-delimited and tab-delimited files (CSV, TSV)     :gnu:
...
** dockerfile-mode for editing Dockerfiles                          :nongnu:
...
*** json-mode for editing JSON files                                   :gnu:
...
** Buffer reformatter macro                                         :nongnu:
...
*** flymake-quickdef for defining Flymake backends                 :vendored:
...
*** Use ELPA version of python.el                             :disabled:gnu:
...
Additionally, add to ~$HOME/.gnupg/gpg.conf~ the line:

#+begin_src
pinentry-mode loopback
#+end_src

And add to ~$HOME/.gnu/gpg-agent.conf~ the line:

#+begin_src
allow-loopback-pinentry
#+end_src

See [[https://bugs.launchpad.net/ubuntu/+source/pinentry/+bug/1986895][here]] for more info.
...
** wgrep for writable grep buffers with changes pushed to files       :nongnu:
...
** Disable global font-locking (syntax highlight) by default          :early:

#+name: no-default-font-lock
#+begin_src emacs-lisp
...
** Add frame internal border                                       :disabled:
...
** Hide lines that are too long                                    :vendored:
...
*** Elpher Gopher and Gemini client                                 :nongnu:
...
** xr for converting Emacs regexps to rx notation                      :gnu:
...
#+name: early-init.el
#+begin_src emacs-lisp :noweb no-export :tangle stow/emacs/dot-emacs.d/early-init.el
...
#+name: init.el
#+begin_src emacs-lisp :noweb no-export :tangle stow/emacs/dot-emacs.d/init.el
